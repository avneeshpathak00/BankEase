...
public class ValidationUtil {
	...
	public static boolean isValidPhoneNumber(String phoneNumber, String countryCode) {
		...
		try {
			...
			throw new UserInvalidException(
					String.format(ApiMessages.USER_PHONE_NUMBER_INVALID_ERROR.getMessage(), phoneNumber, countryCode));
		}
	...
	public static void validateUserDetails(User user) {
		...
		if (!isValidEmail(user.getEmail())) {
			throw new UserInvalidException(
					String.format(ApiMessages.USER_EMAIL_ADDRESS_INVALID_ERROR.getMessage(), user.getEmail()));
		}

		if (!isValidCountryCode(user.getCountryCode())) {
			throw new UserInvalidException(
					String.format(ApiMessages.USER_COUNTRY_CODE_INVALID_ERROR.getMessage(), user.getCountryCode()));
		}

		if (!isValidPhoneNumber(user.getPhoneNumber(), user.getCountryCode())) {
			throw new UserInvalidException(String.format(ApiMessages.USER_PHONE_NUMBER_INVALID_ERROR.getMessage(),
					user.getPhoneNumber(), user.getCountryCode()));
		}
